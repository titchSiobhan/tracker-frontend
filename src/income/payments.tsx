...
function Payments() {
	...
	const [searchValue, setSearchValue] = useState('dateCreated');
	...
	const filteredInvoices = invoices.filter((invoice) => {
		let isoDate;
		if (searchValue === 'dateCreated') {
			isoDate = invoice.dateCreated.slice(0, 10).split('/').reverse().join('-');
		} else {
			isoDate = invoice?.paidDate;
		}
	...
	const total = filteredInvoices.reduce(
		(acc, invoice) =>
			...
			invoice.tasks.reduce(
				(acc, task) => acc + task.task.pricePerUnit * task.quantity,
				0,
			),
		0,
	);
			...
			<div className="invoice-filter">
				...
				<div id="invoice-list">
					{startDate && endDate ? (
						<>
							<p>
								Dates between {startDate.split('-').reverse().join('-')} and{' '}
